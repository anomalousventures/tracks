...
**Note:** GitHub issues will be created after this epic document is approved.

1. **Create commands/ directory structure**
2. **Move newCmd to internal/cli/commands/new.go**
3. **Move versionCmd to internal/cli/commands/version.go**
4. **Create internal/cli/interfaces/ directory**
5. **Move Validator interface to internal/cli/interfaces/validator.go**
6. **Move ProjectGenerator interface to internal/cli/interfaces/generator.go**
7. **Fix validator → cli import: pass logger as constructor param**
8. **Write unit tests for interface moves**

### Phase 2: Dependency Injection Pattern (Can Start After Phase 1)

Establish the pattern we'll use throughout.

1. **Create NewCommand struct with DI fields**
2. **Implement NewNewCommand constructor**
3. **Create VersionCommand struct with DI**
4. **Update root.go to use command factories**
5. **Write unit tests for NewCommand**
6. **Write unit tests for VersionCommand**

### Phase 3: Package Restructuring (Can Parallelize with Phase 2)

Clean separation of concerns.

1. **Create internal/validation/ package**
2. **Move validator implementation to internal/validation/**
3. **Update imports across codebase**
4. **Write tests for validation package**

### Phase 4: Context Propagation (After Phase 2)

Consistent pattern for request-scoped values.

1. **Establish context propagation pattern**
2. **Attach logger to context in root**
3. **Update command signatures to receive context**
4. **Write tests for context propagation**

### Phase 5: Documentation Updates (After Phase 4)

Make documentation match reality.

1. **Update CLAUDE.md CLI architecture section**
2. **Update Epic 3 to reflect new structure**
3. **Update Epic 1 (completed work) for accuracy**
...
1. **Add import cycle detection test**
2. **Add interface location validation test**
3. **Add DI pattern enforcement test**
4. **Document architecture test strategy**
