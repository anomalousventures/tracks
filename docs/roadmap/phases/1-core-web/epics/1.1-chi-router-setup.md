...
16. **Generate route constants template in internal/templates/**
    - Template for routes.go generation
    - Placeholder routes (health, home, about)
    - Comment documentation

17. **Generate server.go template**
    - Server struct template
    - Builder pattern methods
    - Route registration skeleton

18. **Generate main.go template for cmd/server/**
    - Configuration loading
    - Signal handling
    - Server initialization and shutdown

19. **Generate HTTP config template for .env.example**
    - All HTTP configuration options
    - Sensible defaults
    - Documentation comments

20. **Add route helper generation to ProjectGenerator**
    - Generate routes.go during `tracks new`
    - Generate server.go during `tracks new`
    - Generate main.go during `tracks new`

21. **Update Makefile template with server targets**
    - `run-server` target
    - `dev` target (calls Air when configured)
    - `build-server` target

22. **Document Chi router setup in generated README**
...
23. **Add request timeout middleware (chi built-in)**
    - Use `chi.Timeout()` middleware
    - Configure timeout from environment
    - Document timeout behavior

24. **Add max body size configuration**
    - Limit request body size
    - Return 413 Payload Too Large
    - Configure via environment variable

25. **Implement structured logging integration point**
    - Logger field in Server struct
    - `WithLogger()` builder method
    - Log server lifecycle events

26. **Add server shutdown timeout configuration**
    - Configurable via environment variable
    - Default to 30 seconds
    - Log forced shutdown if timeout exceeded

27. **Create integration test for server lifecycle**
    - Start server in test
    - Make HTTP request to /health
    - Trigger shutdown
    - Verify graceful shutdown

28. **Add HTTP/2 support**
...
30. **Create Chi router troubleshooting guide**
