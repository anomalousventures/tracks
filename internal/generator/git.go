...
func InitializeGit(projectPath string, skipGit bool) error {
	...
	if err := runGitCommand(projectPath, "config", "--local", "user.name", "Tracks"); err != nil {
		return fmt.Errorf("failed to configure git user name: %w", err)
	}

	if err := runGitCommand(projectPath, "config", "--local", "user.email", "tracks@tracks.local"); err != nil {
